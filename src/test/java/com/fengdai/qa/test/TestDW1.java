...
public class TestDW1 {
	...
	public static String getmiwen(String var) {
		String var1=JSONObject.toJSONString(CaseUtil.getmingwen(var, "DWERP@#12$3458ta"));
...
//		getmiwen(var);
		...
		requestbody.put("time", CaseUtil.getcurrenttime());
		requestbody.put("token", "");
		requestbody.put("content", contentMap);
		System.out.println(CaseUtil.handlerdewein(JSONObject.toJSONString(requestbody),"DWERP@#12$3458ta"));
		ResponseBody responsebody = given().body(CaseUtil.handlerdewein(JSONObject.toJSONString(requestbody),"DWERP@#12$3458ta")).post("apiservice").getBody();
		String token = responsebody.path("token");
		HashMap contentresponsejiemi = CaseUtil.getmingwen(responsebody.path("content"), "DWERP@#12$3458ta");
